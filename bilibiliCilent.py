...
class bilibiliClient():
    ...
    def __init__(self, roomid, area, danmaku_parser=None):
        ...
        self._roomId = roomid
        self.area = area
        self.danmaku_parser = danmaku_parser
    ...
    async def ReadSocketData(self, len_wanted):
        ...
        while len_remain != 0:
            ...
            try:
                ...
                Printer().printer(f"{sys.exc_info()[0]}, {sys.exc_info()[1]} @[{self.area}分区]{self._roomId}", "Error",
                                  "red")
    ...
    async def parse_packet(self, packet_len: int, body: bytes):
        ...
        if action == 3:
            ...
            Printer().printer(f"self._UserCount = {self._UserCount}", "Warning", "yellow")
        else:
            try:
                dic = json.loads(body)
    ...
    async def parseDanMu(self, dic):
        ...
        if cmd is None:
            ...
            return
        trigger_cmds = ["WELCOME", "SEND_GIFT", "DANMU_MSG", 'LIVE', 'PREPARING']
        if cmd in trigger_cmds and self.danmaku_parser is not None:
            asyncio.create_task(self.danmaku_parser(dic))
            return
        if cmd == 'LIVE':
            ...
            # Printer().printer(f"{dic}", "Message", "cyan", printable=True)
            pass
        elif cmd == 'SYS_GIFT':
        ...
        # 礼物效果 [连击开始，连击结束，使用积分加成卡]
            ...
            pass
        # PK相关
        elif cmd in ["PK_INVITE_INIT", "PK_INVITE_FAIL", "PK_INVITE_CANCEL", "PK_INVITE_SWITCH_OPEN",
        ...
        elif cmd in ["ROOM_SHIELD", "ROOM_BLOCK_MSG", "ROOM_SILENT_ON", "ROOM_SILENT_OFF", "room_admin_entrance",
                     "ROOM_ADMINS"]:
            pass
        ...
        elif cmd in ["CHANGE_ROOM_INFO", "ROOM_SKIN_MSG", "WISH_BOTTLE", "ROOM_REAL_TIME_MESSAGE_UPDATE", "ROOM_CHANGE",
                     "BOX_ACTIVITY_START", "WIN_ACTIVITY", "DANMU_LOTTERY_END"]:
            pass
        # 大乱斗活动
        elif cmd in ["PK_BATTLE_ENTRANCE", "PK_BATTLE_PRE", "PK_BATTLE_MATCH_TIMEOUT", "PK_BATTLE_START",
                     "PK_BATTLE_VOTES_ADD",
                     "PK_BATTLE_PROCESS", "PK_BATTLE_PRO_TYPE", "PK_BATTLE_GIFT", "PK_BATTLE_END",
